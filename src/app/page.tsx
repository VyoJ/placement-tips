...
export default function Home() {
  const [latestTips, setLatestTips] = useState<Tip[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [tipsResponse, companiesResponse] = await Promise.all([
          axios.get('/api/latest-tips'),
          axios.get('/api/featured-companies')
        ]);
        setLatestTips(tipsResponse.data);
          ...
          <Card className="hover:shadow-lg transition-shadow dark:bg-gray-800">
            ...
            <CardContent>
              <ul className="space-y-2">
                {latestTips.map((tip) => (
                  <li key={tip._id} className="flex items-center">
                    <ChevronRight className="mr-2 h-4 w-4 text-blue-500 dark:text-blue-400" />
                    <span className="dark:text-gray-200">{tip.title}</span>
                  </li>
                ))}
              </ul>
            </CardContent>
